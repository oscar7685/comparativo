...
public class sqlConnection {
    ...
    //+
    public void conectarMySQL(String bd, String host, String user, String pass) {
        String maquina = host;
        String nombreBD = bd;
        String usuario = user;
        String clave = pass;
        ...
        try {
            String url = "jdbc:postgresql://" + maquina + ":5432/" + nombreBD;
            conexion = (Connection) DriverManager.getConnection(url, usuario, clave);
           // System.out.println("conectado a " + bd);
        } catch (SQLException e) {
            error = "'conectar POSTGRESQL()' [Error al intentar conectarse. "
                    + (String) e.getMessage() + "]";
            System.err.println(error);
            ste = e.getStackTrace();
            e.printStackTrace();
